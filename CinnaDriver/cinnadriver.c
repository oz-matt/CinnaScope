...
#define DEVICENAME "cinnadevice"

/*inode reffers to the actual file on disk*/
static int device_open(struct inode *inode, struct file *filp) {
    if(down_interruptible(&sem) != 0) {
        printk(KERN_ALERT "cinnadevice : the device has been opened by some other device, unable to open lock\n");
        return -1;
    }
    //buff_rptr = buff_wptr = device_buffer;
    printk(KERN_INFO "cinnadevice : device opened succesfully\n");
    return 0;
}
...
static ssize_t device_read(struct file *fp, char *buff, size_t length, loff_t *ppos) {
    ...
    printk(KERN_INFO "cinnadevice : device has been read %d\n",bytes_read);
    *ppos += bytes_read;
    printk(KERN_INFO "cinnadevice : device has been read\n");
    return bytes_read;
}
...
static ssize_t device_write(struct file *fp, const char *buff, size_t length, loff_t *ppos) {
    ...
    printk(KERN_INFO "cinnadevice : device has been written %d\n",bytes_writen);
    *ppos += bytes_writen;
    printk(KERN_INFO "cinnadevice : device has been written\n");
    return bytes_writen;
}

static loff_t device_lseek(struct file *file, loff_t offset, int orig) {
    loff_t new_pos = 0;
    printk(KERN_INFO "cinnadevice : lseek function in work\n");
    switch(orig) {
        case 0 : /*seek set*/
            new_pos = offset;
            break;
...
static int device_close(struct inode *inode, struct file *filp) {
    up(&sem);
    printk(KERN_INFO "cinnadevice : device has been closed\n");
    return ret;
}
...
int chardev_init(void) 
{
    ...
    if(ret < 0) {
        printk(KERN_ALERT " cinnadevice : failed to allocate major number\n");
        return ret;
    }
    else
        printk(KERN_INFO " cinnadevice : major number allocated succesful\n");
    major_number = MAJOR(dev_num);
    printk(KERN_INFO "cinnadevice : major number of our device is %d\n",major_number);
    printk(KERN_INFO "cinnadevice : to use mknod /dev/%s c %d 0\n",DEVICENAME,major_number);
    ...
    if(ret < 0) {
        printk(KERN_ALERT "cinnadevice : device adding to the kerknel failed\n");
        return ret;
    }
    else
        printk(KERN_INFO "cinnadevice : device additin to the kernel succesful\n");
...
void chardev_exit(void)
{
    ...
    printk(KERN_INFO " cinnadevice : removed the mcdev from kernel\n");

    unregister_chrdev_region(dev_num,1);
    printk(KERN_INFO "cinnadevice : unregistered the device numbers\n");
    printk(KERN_ALERT " cinnadevice : character driver is exiting\n");
}
